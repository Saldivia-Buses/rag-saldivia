...
### Added
- Internal messaging system with channels, DMs, threads, reactions, @mentions, pins (Plan 25)
- Messaging UI: ChannelList, ChannelHeader, MessageList, MessageItem, MessageComposer, ThreadPanel (Plan 25)
- ChannelCreateDialog and DirectMessageDialog for creating channels and DMs (Plan 25)
- MessageActions: edit, delete, copy, reply in thread with permission checks (Plan 25)
- ReactionPicker with 30 native emojis, reaction chips on messages (Plan 25)
- MentionSuggestions dropdown on "@" with blue highlight rendering in messages (Plan 25)
- PinnedMessages side drawer for pinned messages per channel (Plan 25)
- PresenceIndicator (online/away/offline dot) and TypingIndicator with animated dots (Plan 25)
- CommandPalette (Cmd+K) with FTS5 message search and channel navigation (Plan 25)
- VoiceInput speech-to-text for message composer (es-AR) (Plan 25)
- useGlobalHotkeys: Cmd+K (palette), Cmd+N (new channel) (Plan 25)
- File upload API route with 10MB limit and FileAttachment component (Plan 25)
- Bun WebSocket sidecar server on :3001 with JWT auth, pub/sub, presence (Plan 25)
- WebSocket client hooks: useMessaging, usePresence, useTyping (Plan 25)
- Messaging API routes: channels CRUD, messages CRUD, reactions, pins, search, upload, members (Plan 25)
- 10 server actions for messaging with authAction middleware (Plan 25)
- BullMQ messaging-notifications worker for @mentions, DMs, thread replies (Plan 25)
- SQLite FTS5 virtual table for full-text message search with sync triggers (Plan 25)
- Drizzle migration 0002: messaging tables (channels, channel_members, msg_messages, msg_reactions, msg_mentions, pinned_messages) (Plan 25)
- Messaging Zod schemas: Channel, Message, Reaction, WebSocket protocol (Plan 25)
- NavRail: messaging link with MessagesSquare icon (Plan 25)
- UnreadBadge component for unread message counts (Plan 25)
- `data/messaging-files/` and `dump.rdb` added to .gitignore (Plan 25)
- Expanded commitlint scopes: messaging, settings, roles, areas, permissions, ws, queue, workers (Plan 25)
