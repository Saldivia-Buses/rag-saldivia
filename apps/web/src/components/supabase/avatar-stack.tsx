    ...
    >
      {shownAvatars.map(({ name, image }, index) => (
        <Tooltip key={`${name}-${image}-${index}`}>
          <TooltipTrigger render={<Avatar className="hover:z-10" />}><AvatarImage src={image} /><AvatarFallback>
                              {name
                                ?.split(' ')
                                ?.map((word) => word[0])
                                ?.join('')
                                ?.toUpperCase()}
                            </AvatarFallback></TooltipTrigger>
          <TooltipContent>
            <p>{name}</p>
          </TooltipContent>
        </Tooltip>
      ...
      {hiddenAvatars.length ? (
        <Tooltip key="hidden-avatars">
          <TooltipTrigger render={<Avatar />}><AvatarFallback>+{avatars.length - shownAvatars.length}</AvatarFallback></TooltipTrigger>
          <TooltipContent>
