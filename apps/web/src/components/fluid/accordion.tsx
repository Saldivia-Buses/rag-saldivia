...
import { fontWeights } from "@/components/fluid/font-weight";
import { useProximityHover } from "@/components/fluid/use-proximity-hover";
  ...
  ({ children, className, ...props }, ref) => {
    ...
    return (
      <AnimatePresence initial={false}>
        {isOpen && (
          <AccordionPrimitive.Content forceMount {...props} render={<motion.div ref={ref} className={cn("overflow-hidden cursor-pointer", className)} onClick={onToggle} initial={{ height: 0 }} animate={{ height: "auto" }} exit={{ height: 0 }} transition={springs.moderate} onUpdate={() => {
                                groupCtx?.remeasure();
                              }} onAnimationComplete={() => {
                                groupCtx?.remeasure();
