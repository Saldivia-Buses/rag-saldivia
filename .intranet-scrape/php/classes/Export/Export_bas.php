...
class Export_bas extends Export{
    

    public function prepareFile(){
        
        $this->separator = $this->Container->delimitador;
    }

    public function header(){
        /*
                if ($y == 1){
            $tabla= substr($MisDatos->TablaBase, 0, 3);
            $col = strtoupper($tabla.':'.$nomcampo);
            $salidaTit .=$separator.'"'.$col.'"';
        }
    ...
    public function processData($row, $Field, $params){
        ...
        $xsdType= Types::getTypeXSD($Field->TipoDato);
        switch ($xsdType) {
            case "xsd:integer" :
                if ($valorxls  == '') $valorxls = 0;
                if ($valor  == '') $valor = 0;
                $quotes ='';
            break;
            case "xsd:decimal" :
        ...
        $this->output .= $this->separator.$quotes.$valorxls.$quotes ;
    ...
    public function sendHeaders(){
        ...
        header("Content-Disposition:attachment; filename=\"".$this->filename.".bas\"");        
