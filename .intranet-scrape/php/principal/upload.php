...
// Settings
	$save_path = $_POST["DIR"];				// The path were we will save the file (getcwd() may not be reliable and should be tested in your environment)
...
// Validate that we won't over-write an existing file
	if (file_exists($save_path . $file_name)) {
		...
		$md5Old= md5_file($save_path . $file_name);
		$md5New= md5_file($_FILES[$upload_name]['tmp_name']);
		...
		if ($md5Old != $md5New){
			...
			$hiddenFolder = $save_path.'.'.$file_name;
			$filetime = date('Y_m_d_H:i:s',filemtime  ( $save_path . $file_name  ));
			$newFileName  = $baseFileName.'_'.$filetime.'.'.$extension;
			mkdir($hiddenFolder);
			rename($save_path . $file_name, $hiddenFolder.'/'. $newFileName);
		}
                else {
