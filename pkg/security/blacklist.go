...
// Currently includes a Redis-backed JWT token blacklist for logout/revocation.
package security
...
// When a user logs out, the token's JTI (JWT ID) is added to the blacklist
// with a TTL matching the token's remaining lifetime. This ensures revoked
// tokens are rejected even though JWTs are stateless.
...
// Revoke adds a token ID to the blacklist. The entry expires when the
// token would have expired naturally, so the blacklist doesn't grow forever.
